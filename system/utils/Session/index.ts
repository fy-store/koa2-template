...
import { Config, Eatch, JSONData } from './types/index.js'
...
export default class Session {
	...
	#cloneData<T extends JSONData>(content: T): T {
	 ...
	 * @returns 更新后的会话
	 */
	async update(id: string, prop: PropertyPath, value: JSONData | number | string | boolean | null) {
		const type = isType(value)
		let content: unknown
		if (type === 'array' || type === 'object') {
			content = this.#cloneData(value as JSONData)
		} else {
		...
		if (this.#config.onUpdate) {
			...
			await this.#config.onUpdate.call(this, {
				id,
				prop,
				value: content as string | number | boolean | JSONData,
				originData,
				get newData() {
					if (!newData) {
	 ...
	 * @returns 新的会话
	 */
	async set(id: string, data: JSONData) {
	 ...
	 * @returns 会话 ID
	 */
	load(id: string, content: JSONData) {
		if (this.get(id)) {
